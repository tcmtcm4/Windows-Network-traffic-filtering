...
void CSafeguardDlg::OnApply() 
{
	...
	for(unsigned int i=0;i<Passthru.size();i++)
	{
		...
		test<<temp.sourcePort<<"   "<<temp.destinationPort<<endl;
		test<<temp.destinationIP%256<<(temp.destinationIP/256)%256<<(temp.destinationIP/256/256)%256<<(temp.destinationIP/256/256/256)%256<<endl;
		test<<temp.sourceIP%256<<(temp.sourceIP/256)%256<<(temp.sourceIP/256/256)%256<<(temp.sourceIP/256/256/256)%256<<endl;
		test<<temp.sourceMask<<"  "<<temp.destinationMask<<endl;
		if(temp.bDrop == false){  
			test << "拒绝";
		}else{
			test << "通过";
		}
		test<<"  "<<temp.protocol<<endl;
		IMSetRules(&temp,1);
	}
}
